...
const generateMockStats = (timeRange: string) => {
  ...
  const totalEmailsSent = emailsOverTime.reduce((sum, item) => sum + item.count, 0);
  ...
  return {
    ...
    emailsPerTemplate: [
      { templateName: 'Welcome Email', count: Math.floor(totalEmailsSent * 0.3) },
      { templateName: 'Newsletter', count: Math.floor(totalEmailsSent * 0.25) },
      { templateName: 'Promotion', count: Math.floor(totalEmailsSent * 0.2) },
      { templateName: 'Password Reset', count: Math.floor(totalEmailsSent * 0.15) },
