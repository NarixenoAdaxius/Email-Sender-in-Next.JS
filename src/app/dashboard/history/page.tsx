...
import { SearchIcon, RefreshCw, EyeIcon, ChevronLeftIcon, ChevronRightIcon } from 'lucide-react';
...
export default function EmailHistoryPage() {
  ...
  const fetchEmailHistory = async (page = 1, search = searchQuery, status = statusFilter) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      const emailsWithTemplateNames = await Promise.all(
        data.emails.map(async (email: EmailData) => {
          ...
          try {
            const templateRes = await fetch(`/api/templates/${email.templateId}`);
            if (templateRes.ok) {
              ...
              {isLoading ? (
                ...
                emails.map((email) => (
                  <TableRow key={email.id}>
                    ...
                    <TableCell>{email.recipients && email.recipients.length > 1 
              ...
              <div className="mt-4 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1 font-medium">{selectedEmail.template || 'Unknown Template'}</div>
                  </div>
                </div>
                ...
                <div className="rounded-md border p-4 mt-4">
                  ...
                  <div className="p-4 bg-gray-50 rounded-md min-h-[200px] max-h-[400px] overflow-auto">
                    <div className="border-b pb-2 mb-4">
                      ...
                      <div className="text-sm text-muted-foreground">
                        To: {selectedEmail.recipients?.join(', ') || selectedEmail.recipient}
                      </div>
                    </div>
                    {isLoadingDetails ? (
