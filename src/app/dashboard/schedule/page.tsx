...
import { Calendar, Clock, Plus, Trash2, Edit, Calendar as CalendarIcon } from 'lucide-react';
...
export default function EmailSchedulePage() {
  ...
  const handleAddScheduledEmail = async () => {
    // Validate form
    if (!newEmail.subject || (!newEmail.recipientGroup && getAllRecipientEmails().length === 0) || 
        !newEmail.scheduledDate || !newEmail.scheduledTime || !newEmail.templateId) {
      toast({
        title: "Error",
  ...
  const filteredContacts = contactsSearchQuery 
    ? contacts.filter(contact => 
        ...
        (contact.company && contact.company.toLowerCase().includes(contactsSearchQuery.toLowerCase()))
      )
    : contacts;
                ...
                <div className="col-span-3 space-y-4">
                  <div className="flex flex-col space-x-2">
                    <div className="flex space-x-2">
                      <Button 
                        type="button" 
                        variant={newEmail.recipientGroup ? "default" : "outline"}
                        className="w-1/2"
                        onClick={() => {
                    ...
                    {newEmail.recipientGroup || !showContactsList ? (
                      <Select 
                        ...
                        onChange={(e) => setNewEmail({ ...newEmail, recipientGroup: e.target.value })}
                      >
                        ...
                        <div className="border rounded-md p-3 bg-gray-50">
                          ...
                          <div className="flex space-x-2 mb-4">
                            <Input
                              ...
                              onChange={(e) => setNewManualRecipient(e.target.value)}
                              className="flex-grow"
                            />
                            <Button 
                              type="button" 
                            ...
                            <div className="mb-4">
                              ...
                              <div className="flex flex-wrap gap-2">
                                {selectedContacts.map(contact => (
                                  <Badge key={contact._id} variant="secondary" className="pl-2 pr-1 py-1 flex items-center">
                                    {contact.name} <span className="text-xs text-gray-500 ml-1">({contact.email})</span>
                                    <Button
                                      type="button"
                                      variant="ghost"
                                      size="sm"
                                ...
                                {manualRecipients.map(email => (
                                  <Badge key={email} variant="outline" className="pl-2 pr-1 py-1 flex items-center">
                                    {email}
                                    <Button
                                      type="button"
                                      variant="ghost"
                          ...
                          <div>
                            <h4 className="text-sm font-medium mb-2">Browse Contacts:</h4>
                            <Input
                              type="text"
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <div className="col-span-3">
                  <select
                    ...
                    onChange={(e) => setNewEmail({ ...newEmail, recurrence: e.target.value })}
                    className="w-full rounded-md border border-input bg-background px-3 py-2"
                  >
