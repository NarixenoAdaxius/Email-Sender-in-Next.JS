...
export function EmailPreview({ blocks, activeBlock, onSelectBlock, onMoveBlock, onRemoveBlock }: EmailPreviewProps) {
  ...
  const getSocialIcon = (platform: string) => {
    switch(platform) {
      ...
      case 'linkedin': return <FaLinkedin size={24} color="#0077B5" />;
      case 'youtube': return <FaYoutube size={24} color="#FF0000" />;
      case 'pinterest': return <FaPinterest size={24} color="#BD081C" />;
      case 'tiktok': return <FaTiktok size={24} color="#000000" />;
      case 'snapchat': return <FaSnapchat size={24} color="#FFFC00" />;
      case 'github': return <FaGithub size={24} color="#333333" />;
      case 'discord': return <FaDiscord size={24} color="#5865F2" />;
      case 'reddit': return <FaReddit size={24} color="#FF4500" />;
      case 'twitch': return <FaTwitch size={24} color="#9146FF" />;
      case 'whatsapp': return <FaWhatsapp size={24} color="#25D366" />;
      case 'telegram': return <FaTelegram size={24} color="#0088CC" />;
      case 'website':
      default: return <FaGlobe size={24} color="#4285F4" />;
    }
  };
            ...
            >
              ...
              {block.type === 'divider' && (
                <div style={{ padding: '10px' }}>
                  <hr style={{ border: 'none', height: '1px', backgroundColor: '#eeeeee' }} />
                </div>
              )}
              ...
              {block.type === 'quote' && (
                <blockquote style={block.styles as React.CSSProperties}>
                  ...
                  {block.content.quoteAuthor && (
                    <footer style={{ marginTop: '10px', fontStyle: 'normal', fontWeight: 'bold' }}>
                      — {block.content.quoteAuthor}
                    </footer>
                  )}
                </blockquote>
              ...
              {block.type.startsWith('grid-') && (
                <div>
                  <table width="100%" cellPadding="0" cellSpacing="0" border={0} style={block.styles as React.CSSProperties}>
                    <tbody>
                      <tr>
                        {Array(block.type === 'grid-1-column' ? 1 : 
                              block.type === 'grid-2-column' ? 2 : 3)
                          .fill(0).map((_, i) => (
                          <td key={i} width={`${100 / (block.type === 'grid-1-column' ? 1 : block.type === 'grid-2-column' ? 2 : 3)}%`} 
                            style={{ 
                              padding: '10px',
                              verticalAlign: 'top',
                            ...
                            {block.content.columns && 
                             ...
                             block.content.columns[i].length > 0 ? (
                              <div className="space-y-3">
                                {block.content.columns[i].map((columnBlock) => (
                                  <div key={columnBlock.id} className="mb-3 relative">
                                    {/* Reuse the same rendering logic for different block types */}
                                    {columnBlock.type === 'header' && (
                                      <div style={columnBlock.styles as React.CSSProperties}>
                                        <h3>{columnBlock.content.text || ''}</h3>
                                      </div>
                                    )}
                                    
                                    {columnBlock.type === 'text' && (
                                      <div style={columnBlock.styles as React.CSSProperties}>
                                        {(columnBlock.content.text || '').split('\n').map((line: string, i: number) => (
                                          <p key={i}>{line}</p>
                                        ))}
                                      </div>
                                    )}
                                    ...
                                    {columnBlock.type === 'button' && (
                                      <div style={{ textAlign: 'center', padding: '5px' }}>
                                        <a 
                                          href="#" 
                                          style={columnBlock.styles as React.CSSProperties}
                                          onClick={(e) => e.preventDefault()}
                                        >
                                          {columnBlock.content.text || 'Button'}
                                        </a>
                                      </div>
                                    )}
                                    ...
                                    {columnBlock.type === 'image' && (
                                      <div style={{ textAlign: 'center', padding: '5px' }}>
                                        <img 
                                          src={columnBlock.content.src || 'https://via.placeholder.com/300x150'} 
                                          alt={columnBlock.content.alt || ''} 
                                          style={{ maxWidth: '100%', ...columnBlock.styles as React.CSSProperties }} 
                                        />
                                      </div>
                                    )}
                                    
                                    {columnBlock.type === 'spacer' && (
                                      <div style={{ height: columnBlock.content.height || '20px' }}></div>
                                    )}
                                    
                                    {columnBlock.type === 'card' && (
                                      <div style={{ 
                                        padding: columnBlock.content.padding || '16px',
                                        backgroundColor: columnBlock.content.backgroundColor || '#ffffff',
                                        ...
                                        {columnBlock.content.description && (
                                          <p style={{ color: '#4b5563' }}>
                                            {columnBlock.content.description}
                                          </p>
                                        )}
