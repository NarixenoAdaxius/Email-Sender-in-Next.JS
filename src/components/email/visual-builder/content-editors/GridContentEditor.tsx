...
import { PlusIcon, Type, AlignLeft, Link, Image, Layout, Square, MinusIcon, Pencil, Palette } from 'lucide-react';
...
export function GridContentEditor({ block, onUpdateContent, onUpdateStyle }: GridContentEditorProps) {
  const [activeColumn, setActiveColumn] = useState<number | null>(0);
  const [selectedColumnBlock, setSelectedColumnBlock] = useState<TemplateBlock | null>(null);
  const [activeTab, setActiveTab] = useState<'content' | 'style'>('content');
  
  // Extract the column count from the block type
  const getColumnCount = (): number => {
    switch (block.type) {
      case 'grid-1-column': return 1;
      case 'grid-2-column': return 2;
      case 'grid-3-column': return 3;
      default: return 2;
    }
  };
  ...
  useEffect(() => {
    if (!block.content.columns || block.content.columns.length !== columnCount) {
      console.log('Initializing columns', columnCount);
      const initialColumns: TemplateBlock[][] = Array(columnCount).fill(0).map(() => []);
  ...
  const columns = Array.isArray(block.content.columns) && block.content.columns.length > 0 
  ...
  const removeBlockFromColumn = (columnIndex: number, blockIndex: number) => {
    ...
    const newColumns = [...columns];
    newColumns[columnIndex] = newColumns[columnIndex].filter((_, i) => i !== blockIndex);
    
    onUpdateContent(block.id, { columns: newColumns });
    
    if (selectedColumnBlock && 
  ...
  const updateColumnBlockContent = (columnBlock: TemplateBlock, content: Partial<BlockContent>) => {
  ...
  const updateColumnBlockStyle = (columnBlock: TemplateBlock, styles: Record<string, string>) => {
    if (!selectedColumnBlock) return;
  ...
  const componentTypes: {type: BlockType; label: string; icon: React.ReactNode}[] = [
    { type: 'text', label: 'Text', icon: <AlignLeft className="h-4 w-4" /> },
    { type: 'button', label: 'Button', icon: <Link className="h-4 w-4" /> },
    { type: 'image', label: 'Image', icon: <Image className="h-4 w-4" /> },
    { type: 'spacer', label: 'Spacer', icon: <MinusIcon className="h-4 w-4" /> },
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <p className="text-sm text-gray-500">
          This is a {columnCount}-column grid block. Click on a column to add content.
        </p>
        {activeColumn !== null && (
          <p className="text-sm font-medium text-blue-600">
            Column {activeColumn + 1} selected. Use the tabs below to add or edit components.
          </p>
        )}
      </div>
          ...
          {Array(columnCount).fill(0).map((_, i) => (
            <div 
              ...
              className={`p-3 min-h-[100px] border cursor-pointer transition-colors ${
                activeColumn === i ? 'bg-blue-50 border-blue-300' : 'hover:bg-gray-50'
              }`}
              onClick={() => {
            ...
            >
              <div className="text-center mb-2 text-sm font-medium">
                Column {i + 1} {columns[i]?.length > 0 ? `(${columns[i].length} items)` : '(Empty)'}
              </div>
                ...
                {columns[i]?.map((columnBlock, blockIndex) => (
                  <div 
                    ...
                    className={`bg-white border rounded-md p-2 flex justify-between items-center ${
                      selectedColumnBlock?.id === columnBlock.id ? 'ring-2 ring-blue-500' : ''
                    }`}
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  >
                    <span className="text-sm truncate flex items-center">
                      <span className="mr-1 text-blue-500">
                        {columnBlock.type === 'text' && <AlignLeft className="h-3 w-3" />}
                        {columnBlock.type === 'button' && <Link className="h-3 w-3" />}
                        {columnBlock.type === 'image' && <Image className="h-3 w-3" />}
                        {columnBlock.type === 'header' && <Type className="h-3 w-3" />}
                        {columnBlock.type === 'spacer' && <MinusIcon className="h-3 w-3" />}
                        {columnBlock.type === 'card' && <Square className="h-3 w-3" />}
                      </span>
                      {columnBlock.type}
                    </span>
      ...
      <div className="w-full border rounded-md">
        <div className="flex border-b">
          <button
            className={`flex-1 py-2 px-4 text-sm font-medium ${
              !selectedColumnBlock ? 'bg-blue-50 text-blue-700' : 'bg-white text-gray-700 hover:bg-gray-50'
            }`}
          ...
          <button
            className={`flex-1 py-2 px-4 text-sm font-medium ${
              selectedColumnBlock ? 'bg-blue-50 text-blue-700' : 'bg-white text-gray-700'
            }`}
        ...
        {!selectedColumnBlock && (
          <div className="p-4 bg-gray-50">
            <Label>Add to Column {activeColumn !== null ? activeColumn + 1 : 1}</Label>
            ...
            <div className="mt-2">
              {activeTab === 'content' ? (
                <ContentEditor 
                  ...
                  onUpdateContent={(id, content) => updateColumnBlockContent(selectedColumnBlock, content)} 
                />
              ) : (
                <StyleEditor
                  block={selectedColumnBlock}
                  onUpdateStyle={(id, styles) => updateColumnBlockStyle(selectedColumnBlock, styles)}
                />
              )}
            </div>
          </div>
